...
def get_seed_ids(file):
  seed_ranges = list(map(lambda n: n.replace("\n", ""), next(file).split(" ")[1:]))
  seed_ids = set()

  for i in range(0, len(seed_ranges), 2):
    start = int(seed_ranges[i])
    delta = int(seed_ranges[i + 1])
    seed_ids.add(start)
    seed_ids.add(start + delta)
...
def parse_input(file):
  ...
  for line in file:
    ...
    if (code.find("map") != -1):
      ...
      input_map[current_map_id].append(list(map(lambda n: int(n), code.split(" "))))
